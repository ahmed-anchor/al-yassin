...
export function Products() {
  ...
  const getProducts = async () => {
    setLoading(true)
    try {
    const response = await axios.post('/api/getProduct' , { productType: false })
              ...
              href={`/products/${!product.categoryName?['faba','tokal'].includes(product.productType)?product.productType:"":product.productType}`}>
                ...
                >
                  ...
                  <div className="p-6 Lalezar">
                    <h3 className="text-3xl font-medium mb-4">{product.categoryName?product.categoryName:product.name}</h3>
                    <p className="text-xl">{
                      ["tokal-pipe","faba-pipe"].includes(product.fileName)?product.description:""
                    }</p>
                  </div>
                </motion.div>
              </Link>
